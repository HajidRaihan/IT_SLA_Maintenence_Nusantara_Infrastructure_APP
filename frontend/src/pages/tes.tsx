...
import {
  ...
  deleteregisBarang,
  getbarang
} from '../api/regisBarangApi';
import { FontAwesomeIcon } from '@fortawesome/react-fontawesome';
import { faPlus } from '@fortawesome/free-solid-svg-icons';
import { toast, ToastContainer } from 'react-toastify';
import 'react-toastify/dist/ReactToastify.css';
import { ItemModal } from '../components/Modals/AddModal';
import { useDisclosure } from '@nextui-org/react';
import { UpdateRegisbarangModal } from '../components/Modals/UpdateLokasiModal';
...
const AddBarang = () => {
  ...
  const handleDelete = async () => {
    try {
      const res = await deleteregisBarang(Barangid);
      // Filter out the deleted location from the state
      setData(data.filter((item) => item.id !== Barangid));
      toast.success('Delete successfully ', res);
    } catch (error) {
      toast.error('Error deleting barang:', error);
    }
  };
  ...
  const handleUpdate = async () => {
    const dataToUpdate = {
      nama_barang: updatebarang,
    };
    try {
      const res = await updateregisBarang(Barangid, dataToUpdate);
      ...
      toast.success('Barang updated successfully :', res);
    } catch (error) {
      toast.error('Error updating barang:', error);
 ...
 {currentItems.map((item, index) => (
          ...
          >
            ...
            <div className="col-span-1 flex items-center">
              <p className="font-medium mr-3 text-black dark:text-white">{item.nama_barang}</p>
            </div>
            <div className="col-span-1 flex items-center">
      ...
      <UpdateRegisbarangModal
        ...
        onAdd={handleUpdate}
        onChange={(e) => setUpdatebarang(e.target.value)}
        value={updatebarang}
