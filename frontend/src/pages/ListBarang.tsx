...
const ListBarang = () => {
  ...
  const {
    isOpen: updateModalMinOpen,
    onOpen: onUpdateModalMinOpen,
    onClose: onUpdateModalMinClose,
  } = useDisclosure();
  ...
  const handleFilter = () => {
    const filtered = data.filter((item) => {
      ...
      const namaEquipmentFilter = newBarang
        ? item.nama_equipment.toLowerCase().includes(newBarang.toLowerCase())
        : true;
      const namaPerusahaanFilter = selectedPerusahan
        ? item.perusahaan
            .toLowerCase()
            .includes(selectedPerusahan.toLowerCase())
        : true;
      const namaMerkFilter = newMerek
        ? item.merk.toLowerCase().includes(newMerek.toLowerCase())
        : true;
  ...
  return (
    <DefaultLayout>
            ...
            <tbody>
              {currentItems.map((item, index) => (
                <tr key={indexOfFirstItem + index}>
                  ...
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                    <p className="text-black dark:text-white">{item.merk}</p>
                  </td>
                  <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
      ...
      <UpdateBarangModal
        ...
        onAdd={handleUpdate}
        onUpdateStock={UpdatehandleStock}
        valueUpdateSpesifikasi={updatedSpesifikasi}
        onUpdatespesifikasi={UpdatedhandleCatatan}
        valueUpdateStock={updateStock}
        onUpdateClose={onUpdateModalClose}
        barangId={BarangId}
      />
      <UpdateBarangModalMin
        isUpdateOpen={updateModalMinOpen}
        onAdd={handleUpdateMinStock}
        onUpdateStock={UpdatehandleStockMin}
        valueUpdateStock={updateStockMin}
